#!/usr/bin/env bash
# OronyxClang — Package + Manifest Generator
...
REPO="${GITHUB_REPOSITORY:-owner/Oronyx_Clang}"
TAG="${RELEASE_TAG:-}"
...
# Fetch LLVM commit message for release notes
LLVM_COMMIT_MSG=""
if [[ -n "$LLVM_COMMIT_FULL" && "$LLVM_COMMIT_FULL" != "unknown" && -d "$LLVM_DIR" ]]; then
  LLVM_COMMIT_MSG=$(git -C "$LLVM_DIR" log -1 --format="%s" 2>/dev/null || echo "")
fi
...
[clang-ver]
OronyxClang $CLANG_VERSION (https://github.com/llvm/llvm-project $LLVM_COMMIT_FULL)
...
# ─── Generate get_latest_url.sh (sourced by installer — only var assignments) ──
URL_SCRIPT="$(dirname "$(dirname "$0")")/get_latest_url.sh"
cat > "$URL_SCRIPT" << EOF
#!/usr/bin/env bash
# Oronyx Clang — Latest Release URL
# This file is auto-generated by package.sh. Source it to get LATEST_URL.
# Security: contains only variable assignments — safe to source from remote.
LATEST_URL="${DOWNLOAD_URL}"
EOF
chmod +x "$URL_SCRIPT"
log "URL script written to $URL_SCRIPT"

# ─── Generate latest.txt (machine-readable single-line URL) ────────────────────
LATEST_TXT="$(dirname "$(dirname "$0")")/latest.txt"
echo "$DOWNLOAD_URL" > "$LATEST_TXT"
log "Latest URL written to $LATEST_TXT"
